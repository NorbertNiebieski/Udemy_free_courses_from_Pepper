...
def log_to_udemy(web_bot, udemy_login, udemy_password):
    ...
    if _is_logged_to_udemy_account(web_bot):
        log.log_and_print("info", "You was already logged to your udemy account")
        return True
    ...
    sleep(web_bot.get_sleep_time() / 2)
    if _is_logged_to_udemy_account(web_bot):
        log.log_and_print("info", "Successfully logged into your udemy account")
        return True
    else:
        log.log_and_print("info", "Unable to login to your udemy account")
        return False
...
def _check_is_perimeterx_blockade_and_try_bypass(web_bot, solve_captcha=True, how_many_trials=10):
    if _is_perimeterx_blockade(web_bot):
        log.log_and_print("info", "Detected PerimeterX blockade")
        sleep(web_bot.get_sleep_time() / 2)

        # try to solve captcha how_many_trails times
        while solve_captcha and how_many_trials >= 0:
            log.log_and_print("info", "Trying to solve captcha")

            if _try_solve_perimeterx_captcha(web_bot):
                log.log_and_print("info", "Successfully bypass PerimeterX blockade")
                return False

            log.log_and_print("info", "Unable to solve PerimeterX captcha")
            how_many_trials -= 1

        log.log_and_print("info", "Unable to bypass PerimeterX blockade")
        return True
    else:
        return False
...
def _check_cloudflare_blockade_and_try_bypass(web_bot, ask_human_for_help=True, restart_if_blockade=True):
    if _is_cloudflare_blockade(web_bot):
        log.log_and_print("info", "Detected Cloudflare blockade")
        ...
        if ask_human_for_help:
            ...
            if not _is_cloudflare_blockade(web_bot):
                log.log_and_print("info", "Successfully bypass Cloudflare blockade")
                return False

        # restart whole program and hope some randomization of data help
        if restart_if_blockade:
            log.log_and_print("info", "Randomizing data and restarting")
            web_bot.driver.quit()
            subprocess.call([sys.executable, os.path.realpath(web_bot.starting_file)] + sys.argv[1:])

        log.log_and_print("info", "Unable to bypass Cloudflare blockade")
        return True
    else:
        return False
...
def buy_free_course(web_bot, udemy_link, course_number=0, number_of_course=0):
    ...
    try:
        ...
        log.log_and_print("warning", "Something went wrong when trying check if course is already owned from cache",
                          error)
    ...
    try:
        ...
        log.log_and_print("warning", "Something was wrong when trying to obtain course name", error)
        course_name = "Error"
    ...
    if prize.text == "Zapisz się teraz" or prize.text == "Enroll now":
        ...
        if web_bot.driver.current_url[:50] == "https://www.udemy.com/cart/checkout/express/course":
            ...
            try:
                ...
                log.log_and_print("warning", "Something was wrong, when trying calculate savings for - " + course_name,
                                  error)
            ...
            log.log_and_print("info", "YAY! You have new free course \"" + course_name + "\'!" +
                              how_many_course_left_text)
            return saving

        elif web_bot.driver.current_url[:43] != "https://www.udemy.com/cart/subscribe/course":
            web_bot.number_of_checkout_problem += 1
            log.log_and_print("warning", "I have a problem with this course \"" + course_name + "\" checkout!" +
                              how_many_course_left_text)
            return 0
    ...
    # course not free
        ...
        log.log_and_print("info", "This course \"" + course_name + "\" is not for free!" +
                          how_many_course_left_text)
        return 0
    ...
    # owned course
        ...
        try:
            ...
            log.log_and_print("warning", "Something went wrong when caching owned course information", error)
    ...
    # unknown course
        ...
        log.log_and_print("warning", "I don\'t recognize this course \"" + course_name + "\"" +
                          how_many_course_left_text)
        return 0
...
def _buying_owned_course(course_name, how_many_course_left_text, web_bot):
    ...
    log.log_and_print("info", "You already had course \"" + course_name + "\"!" + how_many_course_left_text)
    return 0
...
def _check_if_course_already_owned(web_bot, udemy_link):
    ...
    if not web_bot.cache_owned_courses:
        ...
        try:
            ...
            log.log_and_print("warning", "File with cached owned courses does not exist", error)
            return False
